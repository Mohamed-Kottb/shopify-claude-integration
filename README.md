...
```
Shopify Store → OAuth install (Railway) → Credentials saved → Claude has 23 Shopify tools
```
...
DELETE /admin/stores/<name>        # Remove a store from Railway volume
...
├── mcp/           # server.ts (stdio) + tools.ts (23 shared tools)
